...
def detect_colors(image, num_colors=20, detect_lip_color=False):
    ...
    if detect_lip_color:
        ...
        if (lip_roi_start_x < image.shape[1] and lip_roi_start_y < image.shape[0] and
            ...
            lip_roi = image[lip_roi_start_y:lip_roi_end_y,
                            lip_roi_start_x:lip_roi_end_x]
...
def main():
    ...
    uploaded_file = st.file_uploader(
        "Upload an image of yourself", type=["jpg", "jpeg", "png"])
    ...
    if uploaded_file is not None:
        ...
        if cropped_image is not None:
            ...
            recommended_colors_url, recommended_colors_palette = recommend_colors(
                season)
            st.markdown(
                f'<a href="{recommended_colors_url}" target="_blank" class="button">Check out the recommended products on Mercari</a>',
            ...
            if recommended_colors_palette:
                color_palette_image = np.zeros(
                    (100, len(recommended_colors_palette) * 100, 3), dtype=np.uint8)
                for i, color in enumerate(recommended_colors_palette):
                    color_palette_image[:, i * 100:(i + 1) * 100] = color
                st.image(color_palette_image, caption="Color Palette",
                         use_column_width=True)
